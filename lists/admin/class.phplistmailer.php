    ...
    public function __construct($messageid, $email, $inBlast = true, $exceptions = false) {
        ...
        if ($GLOBALS['emailsenderplugin']) {
            ...
            if (isset($GLOBALS['phpmailer_smtpuser']) && $GLOBALS['phpmailer_smtpuser'] != '' && isset($GLOBALS['phpmailer_smtppassword']) && $GLOBALS['phpmailer_smtppassword']
                ...
                logEvent('Sending email via ' . $this->Host);
            }
            $this->Mailer = 'smtp';
...
//        $this->addCustomHeader("Return-Receipt-To: ".$GLOBALS["message_envelope"]);
    ...
    $to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '', $envelope = ''
    ) {
...
//      cl_output('filesystem_image_exists '.$docroot.' '.$filename);
        ...
        if (defined('UPLOADIMAGES_DIR')) {
            ...
            return
                    is_file($_SERVER['DOCUMENT_ROOT'] . '/' . UPLOADIMAGES_DIR . '/image/' . $localfile) || is_file($_SERVER['DOCUMENT_ROOT'] . '/' . UPLOADIMAGES_DIR . '/' . $localfile)
                    //# commandline
                    || is_file($imageroot . '/' . $localfile);
        } else {
            return
                    is_file($_SERVER['DOCUMENT_ROOT'] . $GLOBALS['pageroot'] . '/' . FCKIMAGES_DIR . '/image/' . $localfile) || is_file($_SERVER['DOCUMENT_ROOT'] . $GLOBALS['pageroot'] . '/' . FCKIMAGES_DIR . '/' . $localfile)
                    //# commandline
                    || is_file('../' . FCKIMAGES_DIR . '/image/' . $localfile) || is_file('../' . FCKIMAGES_DIR . '/' . $localfile);
        }
    }
...
//      cl_output('get file system image'.$filename.' '.$localfile);
    ...
    public function get_template_image($templateid, $filename) {
        ...
        $req = Sql_Fetch_Row_Query(sprintf('select data from %s where template = %d and (filename = "%s" or filename = "%s")', $GLOBALS['tables']['templateimage'], $templateid, $filename, basename($filename)));
